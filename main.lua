...
-- ==========================================
-- NEW GAME MODE: CHARACTER & MAGIC SYSTEM DATABASE
-- ==========================================
local CharacterData = {
    { name = "Warrior", desc = "High defense, physical damage.", hp = 150, mp = 50, magicElement = "Earth", powers = { "Ground Slam", "Stone Shield" } },
    { name = "Mage", desc = "Master of elements, fragile body.", hp = 80, mp = 200, magicElement = "Fire", powers = { "Fireball", "Flame Burst" } },
    { name = "Rogue", desc = "Fast attacks, shadow magic.", hp = 110, mp = 90, magicElement = "Shadow", powers = { "Shadow Step", "Dark Blade" } },
...
-- ==========================================
local function initTTSEngine()
  if not ttsEngine then
    ttsEngine = TextToSpeech(activity, TextToSpeech.OnInitListener({
      onInit = function(status)
        if status == TextToSpeech.SUCCESS then
          if ttsEngine then ttsEngine.setLanguage(Locale.US) end
        end
      end
    }))
  end
...
local function speakResult(text)
  if ttsEngine then
    pcall(function()
      if Build.VERSION.SDK_INT >= 21 then
        ttsEngine.speak(text, TextToSpeech.QUEUE_FLUSH, nil, nil)
      else
...
local function syncMultiplayerGameOver(winnerRole)
  ...
  if myRole == winnerRole then
    ...
    speakResult("Congratulations! You won the match.")
  else
    losses = losses + 1
    editor.putInt("userLosses", losses)
    txtGameStatus.setText("ðŸ”´ Defeat! Your opponent won.")
    speakResult("Game over! You lost the match.")
  end
  editor.apply()
...
local function showSettingsScreen()
  ...
  btnResetProfile.setOnClickListener(function()
    ...
    confirmDialog.setTitle("ðŸ‘¤ Reset Profile")
    confirmDialog.setMessage("Are you sure you want to clear your profile name?")
    confirmDialog.setPositiveButton("Yes", DialogInterface.OnClickListener({
      onClick = function(d, w)
        local editor = preferences.edit()
        editor.putString("userName", "Player") 
        editor.apply()
        
        local wins = preferences.getInt("userWins", 0)
        local losses = preferences.getInt("userLosses", 0)
        syncDataToFirebase("Player", wins, losses)
  ...
  setLayout.addView(btnResetProfile)
...
function showMainMenu()
  ...
  btnExit.setOnClickListener(function()
    stopAllMusic(); shutdownTTS(); activity.finish()
  end)
